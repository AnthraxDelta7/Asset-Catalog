...
def _remove_one_asset(
    conn: sqlite3.Connection, thumbnail_dir: Path, assets_dir: Path, asset_id: int
) -> bool:
    row = conn.execute(
...
def remove_assets(
    ...
    for asset_id in asset_ids:
        if _remove_one_asset(conn, thumbnail_dir, assets_dir, asset_id):
            stats.removed += 1
    conn.commit()
    return stats
...
def remove_pack(
    conn: sqlite3.Connection, thumbnail_dir: Path, assets_dir: Path, pack_id: int
) -> RemovePackStats:
