...
Copies selected assets out to a target project into one flat folder per pack (default `exported_assets/<Pack Name>/<filename>` — the pack's own internal subfolder structure, Models/Textures/a creator's own nested layout, isn't reproduced underneath it, on the theory that it's rarely worth preserving once you've already deliberately picked out the handful of assets you're exporting) rather than symlinking — see the design note below for why copy over symlink. Two assets that share a filename in different subfolders of the source pack, which flattening can turn into a genuine collision, get disambiguated (`diffuse.png`, `diffuse (2).png`, ...) rather than one silently overwriting the other. Requires at least one selection filter, or `--all` to export the entire catalogue on purpose (a bare `export <project>` with nothing else is refused, since it's an easy fat-finger away from copying everything):
...
Nothing is lost off the end. The list's own search (the magnifier by the **Packs ›** button) looks past the cap at *every* pack, and **Packs ›** opens the manager, which shows the lot. The search placeholder and the button's tooltip both name the real total so the cap is never a surprise. The pack currently filtering the grid always stays in the list, even when it would otherwise have aged out.

In the pack manager, **Show in List** puts a pack back at the top — it both clears the hidden flag and marks the pack as just-used, so it works for a pack that was merely old as well as one that was hidden. **Hide from List** takes it out; hiding is a listing concern only, and a hidden pack stays catalogued, searchable and exportable.
...
**Export to Godot generates a native Godot asset, not just a copied `.glb`.** Godot's own glTF import produces whatever raw node hierarchy the file happens to have — often a generic root node, sometimes carrying an `AnimationPlayer`/`Skeleton3D` import artifact even for a static prop — awkward to just instance and use, and not something worth hand-configuring per asset via the per-file import "Root Type" override. Only appears at all when the whole current selection is a model (`.glb`/`.gltf`/`.fbx`/`.obj`/`.stl`/`.blend` — a texture, an audio file, or a mixed selection never offers it): the export button's dropdown gains **Export to Godot: `<project>`** entries below the plain ones, and **Export to Project...**'s own dialog gains a **Generate Godot MeshInstance3D scenes** checkbox. Behind the scenes this puts a `.glb` into the project (see the texture note below for how anything that isn't already one gets there), runs one headless Godot import pass plus a scripted pass that walks the imported scene for every `MeshInstance3D` in it, then writes **one of two things depending on how many meshes it found**:

- **One mesh → `<name>.res`, a bare `Mesh` resource.** Drag it into the 3D viewport and Godot creates a plain, local `MeshInstance3D` with it assigned — no scene link, and the same mesh can be reused across as many nodes as you like. A `.tscn` here would instead give an *instanced scene*, tied to its file and needing right-click → **Make Local** before it behaves like an ordinary node, which is the friction this avoids. Since a `Mesh` has nowhere to keep a transform, the source's rotation/scale/position is **baked into the vertex data** on the way out rather than dropped (verified against Godot 4.6: a cube exported with a 45° rotation and a non-uniform `(1, 2, 0.5)` scale comes back with exactly the AABB that transform predicts, material and texture reference intact).
- **More than one mesh → `<name>_meshinstance.tscn`.** A `Node3D` root with one named `MeshInstance3D` child per mesh, each at its own position relative to the others — a scene is the only thing that can express that. Confirmed directly against a real Godot install that the saved scene fully embeds each mesh and material as self-contained sub-resources (not a fragile reference back into the source file's own import) while a texture stays a proper reference to the real image file Godot's own importer already extracts into the project on its own.

- **Anything a flatten would damage → kept whole, as a `.tscn` under a plain `Node3D` root.** glTF carries far more than static geometry, and the rule is an **allowlist, not a blacklist**: a model is flattened only when the file can be seen to hold nothing but plain static geometry, and kept whole otherwise. The two outcomes aren't symmetric — preserving a file that could safely have been flattened costs nothing much, while flattening one that shouldn't have been destroys data you can't recover from what lands in your project. A file is kept whole if it contains a **skeleton**, **animations**, **morph targets** (blend shapes — `SurfaceTool`, which does the flattening, has no blend-shape support at all, so these would vanish silently), **vertex colors**, **a second UV set**, **cameras**, or **any glTF extension not known to survive** the process. A file that can't be parsed is kept whole too: "we couldn't tell what's in here" is never a reason to rewrite something.

  Crucially this is **not** just copying the `.glb` in and leaving you with Godot's import dependency. The whole imported hierarchy — `Skeleton3D`, skinned meshes, `AnimationPlayer` — is re-saved as an ordinary `.tscn` with a plain `Node3D` root, and the `.glb` is deleted like any other intermediate. Confirmed against Godot 4.6 with a real 56 MB, 79-joint, 6-clip character: the saved scene embeds the skeleton, every skin and every animation, referencing only the `.png` files Godot's own importer extracted — the `.glb` and its `.import` were deleted outright and the scene still loaded with all 79 bones, 3 skinned meshes and all 6 clips intact. What lands in your project is an editable Godot scene, not a wrapper around a source file the project has to keep forever.

Either way every mesh keeps its own rotation/scale/position exactly as it was in the source (confirmed with a real glb carrying a genuine scene-graph-level transform, not one already baked into the mesh's own vertex data) — a real bug caught and fixed here: the single-mesh case originally built its new root at the identity transform, silently dropping whatever position/rotation/scale the source actually had. The intermediate model file is then deleted (along with its `.import` residue) once its output is confirmed generated — the `.res`/`.tscn` is the actual deliverable, not a source file sitting next to it — unless generation failed for that one, in which case the source is left in place instead of leaving nothing usable at all.

**A non-`.glb` model is converted through Blender on the way in, so its textures survive.** `.glb` is the only model format that embeds its own textures; `.fbx`, `.obj`, `.gltf`, `.stl` and `.blend` all reference them as sibling files by relative path — so copying one into a project (especially into the one flat folder per pack that export now produces) reliably lands it with its materials pointing at nothing. Export to Godot therefore converts anything that isn't already a `.glb` with Blender first, reading the file **from the staging library**, where it's still sitting next to its own `Textures/` folder; converting a copy that flattening had already separated from those textures would strip exactly what this is meant to preserve. The resulting `.glb` embeds them, and the pack's own corrections (material fallback, broken-texture fallback) get applied on the way through, which no engine-side importer could do. Verified end-to-end against Blender 5.2 and Godot 4.6 with a genuine externally-textured `.fbx`: the finished `.tscn` references a real extracted `.png`, with the source's rotation and scale intact. This is also why `.stl` and `.blend` are offered even though Godot can't read either — Godot never sees them in that form. The catalogue's own copy is never modified; the asset stays the `.fbx` it was, and the `.fbx` is never copied into the project. If Blender isn't installed or configured, the models needing it are reported as failures and any `.glb` in the same selection still exports — deliberately *not* copied in raw, since an untextured model quietly sitting in a project is the exact outcome this avoids.

**Audio needs no Godot mode; use a plain export.** Godot imports `.wav`, `.ogg` and `.mp3` cleanly on its own, so a wrapper scene or hand-tuned `.import` file would be ceremony rather than value — a plain **Export to Project...** already puts the file exactly where it belongs. One real gotcha: **Godot has no FLAC support at all** (confirmed directly against 4.6 — `AudioStreamFLAC` doesn't exist), so a `.flac` exported to a Godot project is a dead file there. Convert it to `.ogg` or `.wav` first.

**Animation preview renders only when you press Play.** An animated asset lists its clips in the detail panel (**animations: @idle, @walk, ...**), and its button reads **View in 3D / Play 6 Animations** so it's clear that's where they're watched. The clip picker and **▶ Play** live in the 3D preview itself, and the animation plays **in place of the viewport** rather than in a second window — the same inline swap the texture preview already uses. Orbiting pauses while it plays (it's a rendered flipbook from a fixed camera); **⏹ Stop** returns to the interactive mesh. Picking a different clip while one is playing cuts straight to it instead of making you stop and start again — instant, since every clip of that model was rendered in the same pass. Frames are rendered against the exact grey the 3D viewport is cleared to and scaled to fill the same pane, so starting playback doesn't flash a different backdrop or shrink the model — the pane just starts moving. Nothing renders until Play is pressed — a rigged character often ships six or more clips, and rendering them at ingest would cost minutes per asset for frames nobody may ever look at.

Once you do press Play, **every clip of that model is rendered in one Blender session**, because importing a large rigged character costs far more than rendering a frame — paying that import once makes the other clips nearly free. Frames come from Blender's **Workbench** renderer rather than EEVEE: this is a motion preview, so reading the movement matters more than lighting fidelity, and Workbench is dramatically faster while still showing textures. Measured on a real 56 MB, 79-joint character: **all six clips in 10.4 seconds** (136 frames), versus roughly a minute for a single clip under EEVEE. Results are cached under the same content-hash identity the thumbnail and interactive preview use, so replaying is instant (0.002s) and identical content is never rendered twice. Playback is a looping flipbook — the interactive preview draws a static mesh via trimesh and has no skinning, so animating there would mean writing joint evaluation and vertex skinning from scratch. A long clip is subsampled to 24 evenly-spaced frames and played proportionally slower so it still runs about its true duration, and camera framing samples several frames rather than only the first, since a walk cycle reaches well outside the bind pose.

**Which export mode is remembered adapts to what's selected, not just to the project.** The button remembers your last *deliberate* choice (standard vs. Godot) as `last_export_mode` in settings, same recent-project list either way — picking a Godot-eligible model after exporting audio to the same project goes right back to Godot mode on its own, and picking that audio asset again temporarily falls back to standard (Godot makes no sense for it) without overwriting the stored preference, so the next eligible model export doesn't need re-picking it.
...
When a newer version exists, a dialog offers **Open Release Page** (opens it in your browser to download manually), **Skip This Version** (remembered in settings — the automatic background check won't mention that specific version again, though a manual check still always reports the real current state), or **Remind Me Later** (does nothing, asks again next launch). The background check fails silently on any error (no network, GitHub unreachable, etc.) — only the manual check surfaces a failure, since a launch-time network hiccup shouldn't interrupt someone who's just trying to catalogue assets.

**One-click download and install is currently switched off** (`SELF_UPDATE_ENABLED` in `main_window.py`), pending real code signing. The mechanism itself (`src/asset_catalogue/self_update.py`, `UpdateDownloadDialog`) is fully implemented and left intact, not deleted: a running `.exe` can't overwrite its own files on Windows, so it downloads the release's `.zip` asset with a live progress bar, extracts it to a temp folder, then hands off to a detached PowerShell script and exits (`os._exit()`, not `sys.exit()` — the latter, called from the Qt slot that triggers this, doesn't cleanly exit at all; PySide6 treats an exception escaping a slot as fatal and aborts the whole process instead). That script waits for this process to actually exit (by PID), renames the current install folder to `<install>.old`, moves the freshly-extracted build into its place (`CREATE_NO_WINDOW` alone in the `subprocess.Popen` that launches it — `CREATE_NO_WINDOW | DETACHED_PROCESS` together silently break classic Windows PowerShell's console host, so the script never even starts running, with nothing raising or failing loudly to say so), relaunches the new `.exe`, and only then deletes the `.old` backup. Every attempt is logged to `%APPDATA%\AssetCatalogue\logs\update-relaunch.log` (30 retries, ~3s apart) — what turned out to still be failing after both of those real bugs were fixed was Windows Defender transiently locking the install folder right as the old unsigned exe exits, confirmed by the folder becoming renamable again moments later by hand. That's expensive to reliably work around unsigned, and exactly the kind of friction a signed build doesn't have, so this stays off until then — flip `SELF_UPDATE_ENABLED` back to re-enable it.

This relaunch script is the one piece of this feature that can never run under pytest (it only executes as a detached process after this app has already exited), so it's been verified with real dry runs instead: copying an actual packaged `dist/AssetCatalogue` build to stand in for both the "old" and "new" install, running the real script end to end, and confirming via `tasklist` that the relaunched real `.exe` came up afterward. That caught a real bug early on — PowerShell requires a line-continuation `+` at the *end* of the preceding line, not the start of the next one; getting it backwards silently fails to parse the *entire* script, including the unrelated wait/rename/move logic above it.
