...
- **Rigged or animated → left as the `.glb` it is.** glTF carries far more than static geometry: a real character ships as one `.glb` holding skinned meshes, a full joint hierarchy and named animation clips. Neither a rig nor an animation can survive being flattened to a plain mesh, so anything carrying one skips the MeshInstance3D step entirely and is copied in untouched, letting Godot's own importer build the `Skeleton3D` and `AnimationPlayer` — strictly better than anything this could synthesize. Detected by reading the glTF's own JSON header (cheap: a 56 MB character is answered without loading the model), and the detail panel shows a matching **contains: rigged (79 joints), 6 animations** line so you know before exporting. Verified against a real Sketchfab-style character pack, which previously came out as three static meshes frozen in bind pose with all six clips gone.
