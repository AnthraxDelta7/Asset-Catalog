...
**Export to Godot generates a clean `MeshInstance3D` scene, not just a copied `.glb`.** Godot's own glTF import produces whatever raw node hierarchy the file happens to have — often a generic root node, sometimes carrying an `AnimationPlayer`/`Skeleton3D` import artifact even for a static prop — awkward to just instance and use, and not something worth hand-configuring per asset via the per-file import "Root Type" override. Only appears at all when the whole current selection is something Godot can actually import on its own (`.glb`/`.gltf`/`.fbx`/`.obj` — a texture, a `.stl`, or a mixed selection never offers it): the export button's dropdown gains **Export to Godot: `<project>`** entries below the plain ones, and **Export to Project...**'s own dialog gains a **Generate Godot MeshInstance3D scenes** checkbox. Behind the scenes this copies the file in as usual, runs one headless Godot import pass plus a scripted pass that walks the imported scene for every `MeshInstance3D` in it (one at the root if there's exactly one, or a `Node3D` root with one child per mesh for more than one), and saves a fresh `<name>_meshinstance.tscn` — confirmed directly against a real Godot 4.4 install that the saved scene fully embeds the mesh and material as self-contained sub-resources (not a fragile reference back into the source file's own import) while a texture stays a proper reference to the real image file Godot's own importer already extracts into the project on its own. Every mesh keeps its own rotation/scale/position exactly as it was in the source (confirmed with a real glb carrying a genuine scene-graph-level transform, not one already baked into the mesh's own vertex data) — a real bug caught and fixed here: the single-mesh case originally built its new root at the identity transform, silently dropping whatever position/rotation/scale the source actually had. The intermediate model file is then deleted (along with its `.import` residue) once its wrapper is confirmed generated — the `.tscn` is the actual deliverable, not a source file sitting next to it — unless generation failed for that one, in which case the source is left in place instead of leaving nothing usable at all.
