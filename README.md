...
**Smart texture matching goes a step further: it tries to recover the *real* texture rather than settle for gray.** Investigating a pack where several assets looked flat/wrong (some pink, some just plain, oddly inconsistent) turned up two distinct, real problems on the same downloaded low-poly kit: one material had a texture reference, but it was a dead absolute path (`F:\Commissions\...`) — the classic broken-link case above; a much larger number of *other* materials had **no texture reference at all**, yet their names (`RecessA`, `PavedFloorB`, `TiledPanelsC`, ...) matched a real texture file sitting in the pack's own `Textures/` folder exactly — the exporter had simply never wired the two together. Meanwhile ~54 *other* material names on the same pack (character/plant/armor parts) had no texture-set counterpart at all and were correctly, legitimately flat-colored. Both cases are addressed automatically, on by default, at import:

- **Relink**: a broken image reference is repointed to a file with the exact same basename found anywhere else in the pack, if one exists.
- **Name-matched injection**: a material with no texture at all gets one wired into Base Color if a texture file's name matches the material's own name by convention (`texture_matching.py`: tokenized, exact whole-token match after stripping a recognized map-type suffix like `_albedo`/`_normal`/`_ao`/`_matid` — never a raw substring, and never a normal/AO/etc.-only match used as a color texture even if it's the only match found).

**A `_BakedAlbedo` file is deliberately never injected, even as the only color candidate.** It's a texture with AO/shadow baked in for one specific mesh's own UV unwrap, but the *material name* it's keyed to is routinely shared by many structurally different meshes across a modular kit (confirmed on the real Sci-Fi pack: one material name was shared by 9 unrelated pieces besides its own dedicated low-poly reference mesh) — injecting it by name onto the wrong one pastes a baked shadow/seam pattern that doesn't correspond to that mesh's real edges, a real defect this shipped with initially and had to be corrected. Worse, its same-named plain `_albedo` file isn't a safe fallback either: every one checked against the real pack turned out to be a leftover UV-checker debug image sharing the bake's exact UV silhouette, not independent art. So a `_BakedAlbedo` sibling's mere presence voids the whole name match — the material is left on its neutral fallback rather than guessing between two untrustworthy files. This only affects injection; a broken-link *relink* still uses a `_BakedAlbedo` file freely, since it's restoring the exact file that specific mesh's own material already named, not guessing across meshes.

Neither pass ever touches a material that already has *any* image texture node, working or broken — this only ever fixes something that was completely wrong or completely absent. Every match is named in the completion message (`Auto-matched N texture(s) by name: 'Wood' -> Wood_albedo.png, ...` for an injection, `relinked Shotgun_albedo.png` for a relink) so an automatic change is never silent, even though it's on by default. Two ways to correct it if a match ever looks wrong:

- **`disable_smart_texture_matching`** (Edit Pack Metadata, or `--disable-smart-texture-matching` on the CLI) turns off both passes for the whole pack — an easy, total rollback.
- **Manual texture overrides** (the same dialog's "Manual texture overrides" list, or `--texture-override MATERIAL=path/relative/to/pack.png` on the CLI, repeatable) pin a specific material to a specific texture file, taking priority over automatic matching even when it's disabled — for pointing the matcher at the right file yourself when the naming convention doesn't line up on its own. The UI's **Add...** button restricts the file picker to the pack's own staging folder, since an override is only meaningful as "a texture that's part of this pack," the same assumption the automatic matcher makes.
