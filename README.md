...
- **First-preview window flicker:** the very first time a `GLViewWidget` appeared anywhere in the process, the whole app would flicker/flash once, as if it had briefly closed and reopened, even though nothing was actually crashing (confirmed: no crash event in Windows' own Application log). The main window is created long before any 3D preview, with a plain non-GL surface; the moment the first OpenGL-backed widget appears, Windows' compositor has to switch that window's backing surface to a GL-capable one on the spot, which is what shows up as a flicker. Fixed by giving every window a GL-capable surface format from the start (`QSurfaceFormat.setDefaultFormat(...)`, set before the `QApplication` is constructed, same timing requirement as `AA_ShareOpenGLContexts` above) — there's nothing left to switch later, since the main window already has a compatible surface from launch.
