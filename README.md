...
**Animation preview renders only when you press Play.** An asset carrying animation clips gets a clip dropdown and a **▶ Play** button in the detail panel. Nothing is rendered when those appear — pressing Play is what runs Blender, once, for that one clip. A rigged character commonly ships six or more clips, and rendering them all during ingest would cost minutes per asset for frames nobody may ever look at. The result is cached under the same content-hash identity the static thumbnail and interactive preview already use, so replaying is instant and identical content is never rendered twice. Playback is a looping frame flipbook rather than real-time 3D: the interactive preview draws a static mesh via trimesh and has no skinning of its own, so animating there would mean writing joint evaluation and vertex skinning from scratch — Blender already does both correctly. A long clip is subsampled to 24 evenly-spaced frames and played back proportionally slower so it still takes about its real duration. Camera framing samples several frames rather than just the first, since a walk cycle reaches well outside the bind pose.
