...
- **Anything a flatten would damage → left as the `.glb` it is.** glTF carries far more than static geometry, and the rule here is an **allowlist, not a blacklist**: a model is flattened only when the file can be seen to hold nothing but plain static geometry, and preserved untouched otherwise. The two outcomes aren't symmetric — preserving a file that could safely have been flattened costs a slightly less convenient import, while flattening one that shouldn't have been destroys data you can't recover from what lands in your project. A file is preserved if it contains a **skeleton**, **animations**, **morph targets** (blend shapes — `SurfaceTool`, which does the flattening, has no blend-shape support at all, so these would vanish silently), **vertex colors**, **a second UV set** (lightmap/detail UVs), **cameras**, or **any glTF extension not known to survive** the process. A file that can't be parsed at all is preserved too: "we couldn't tell what's in here" is never a reason to rewrite something. Preserved files are copied in untouched so Godot's own importer builds the `Skeleton3D` and `AnimationPlayer` — strictly better than anything this could synthesize. Detected by reading the glTF's own JSON header (cheap: a 56 MB character is answered without loading the model), with a second guard inside Godot itself that refuses to flatten a mesh carrying blend shapes and falls back to the scene form if any surface would be lost in the rebuild. The detail panel shows a matching **contains: a 79-joint skeleton, 6 animations** line so you know before exporting, and the export log says exactly why each preserved file was left alone. Verified against a real Sketchfab-style character pack, which previously came out as three static meshes frozen in bind pose with all six clips gone — while ordinary Blender-exported props still flatten as before.
