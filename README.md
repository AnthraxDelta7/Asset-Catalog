...
**Animation preview renders only when you press Play.** An animated asset lists its clips in the detail panel (**animations: @idle, @walk, ...**) alongside a **View in 3D** button; the clip picker and **▶ Play** live in the 3D preview itself, since watching a character run belongs next to the model you can orbit. Nothing renders until Play is pressed — a rigged character often ships six or more clips, and rendering them at ingest would cost minutes per asset for frames nobody may ever look at.
